...
docker pull zhoushu1/truthlens:4.3
...
6. **部署生产化**：CPU 瘦身镜像（torch 换 CPU wheel、去 CUDA，镜像约 2.48GB，容器监听 `0.0.0.0:6655`）、compose 去掉 GPU 保留段、GitHub Actions push 自动构建并推送 Docker Hub（`zhoushu1/truthlens:4.3` + `:latest`）。
